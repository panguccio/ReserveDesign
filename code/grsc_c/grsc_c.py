...
y = grsc.addVars(instance.V, vtype=gb.GRB.BINARY, name="y")
...
grsc.addConstr(gb.quicksum(y[j] for j in instance.V) <= instance.k, name=f"NCOMP")
  

for v in instance.V:
    grsc.addConstr(y[v] <= x[v], name=f"YX")
grsc.optimize()
